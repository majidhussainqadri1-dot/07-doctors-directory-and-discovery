...
final class DDD_Future_Mutation_Guard {
	...
	private static function mutation_scope( WP_REST_Request $request ) {
		...
		if ( 'POST' === $method && preg_match( $base . 'saved-searches/?$#', $route ) ) {
			return 'saved_search_save';
		}
		if ( 'DELETE' === $method && preg_match( $base . 'saved-searches/[a-f0-9]{32}/?$#', $route ) ) {
			return 'saved_search_delete';
		}
		if ( 'POST' === $method && preg_match( $base . 'shortlists/?$#', $route ) ) {
			return 'shortlist_save';
		}
		if ( 'DELETE' === $method && preg_match( $base . 'shortlists/[a-f0-9]{32}/?$#', $route ) ) {
			return 'shortlist_delete';
		}
		return '';
	}
	...
	private static function receipts( $user_id ) {
		...
		foreach ( $rows as $key => $row ) {
			if ( ! is_array( $row ) || absint( $row['expires_at'] ?? 0 ) <= $now ) {
				unset( $rows[ $key ] );
			}
		}
		return $rows;
	...
	private static function acquire_lock( $user_id ) {
		...
		if ( ! is_array( $current ) || empty( $current['expires'] ) || absint( $current['expires'] ) <= time() ) {
			...
			if ( $deleted && add_option( $name, $raw, '', 'no' ) ) {
				return array( 'name' => $name, 'token' => $token );
			}
		}
		return array();
	...
	private static function release_lock( $lock ) {
		...
		$current = json_decode( $current_raw, true );
		if ( ! is_array( $current ) || empty( $current['token'] ) || ! hash_equals( (string) $current['token'], (string) $lock['token'] ) ) {
			return;
		}
		$wpdb->query(
			$wpdb->prepare(
	...
	private static function audit( $scope, $user_id, $outcome, $receipt_hash, $request ) {
		DDD_Repository::audit_admin(
			...
			absint( $user_id ),
			sanitize_key( $scope ),
			substr( sanitize_text_field( (string) $receipt_hash ), 0, 24 ),
	...
	public static function pre_dispatch( $result, $server, $request ) {
		...
		if ( ! $idempotency_key ) {
			return DDD_Helpers::safe_error( 'future_idempotency_key_required', __( 'An Idempotency-Key header is required for this private mutation.', DDD_TEXT_DOMAIN ), 400 );
		}
		...
		if ( ! $lock ) {
			return DDD_Helpers::safe_error( 'future_mutation_busy', __( 'Another private discovery change is still being committed. Retry shortly.', DDD_TEXT_DOMAIN ), 409, array( 'retry_after' => 1 ) );
		}
		...
		if ( isset( $receipts[ $receipt_hash ] ) ) {
			...
			if ( empty( $entry['request_hash'] ) || ! hash_equals( (string) $entry['request_hash'], $request_hash ) ) {
				self::audit( $scope, $user_id, 'conflict', $receipt_hash, $request );
			...
			self::audit( $scope, $user_id, 'replay', $receipt_hash, $request );
			self::release_lock( $lock );
	...
	public static function post_dispatch( $result, $server, $request ) {
		if ( ! self::$active || ! $request instanceof WP_REST_Request ) {
			return $result;
		}
		$ctx = self::$active;
		self::$active = null;
		...
		try {
			if ( self::mutation_scope( $request ) !== $ctx['scope'] || get_current_user_id() !== absint( $ctx['user_id'] ) ) {
				self::audit( $ctx['scope'], $ctx['user_id'], 'context_mismatch', $ctx['receipt_hash'], $request );
				return $result;
			}

			if ( is_wp_error( $result ) ) {
				self::audit( $ctx['scope'], $ctx['user_id'], 'failed', $ctx['receipt_hash'], $request );
				return $result;
			}
			...
			$receipts[ $ctx['receipt_hash'] ] = array(
				'scope'        => $ctx['scope'],
